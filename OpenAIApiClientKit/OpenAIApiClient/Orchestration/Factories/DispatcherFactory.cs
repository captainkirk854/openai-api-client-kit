...
namespace OpenAIApiClient.Orchestration.Factories
{
    ...
    public static class DispatcherFactory
    {
        ...
        public static (SingleAiModelDispatcher, EnsembleDispatcher) Create(IAiModelRegistryX registry)
            => (new SingleAiModelDispatcher(registry: registry), new EnsembleDispatcher(registry: registry));
    }
}